...
export const AgoraProvider: React.FC<AgoraContextType> = ({ children, localCameraTrack, localMicrophoneTrack }) => (
  <AgoraContext.Provider value={{ localCameraTrack, localMicrophoneTrack, children }}>{children}</AgoraContext.Provider>
);
      ...
      <>
        {config.selectedProduct === "live" && (
          <div>
            ...
            <label>
              <input type="radio" value="audience" checked={role === "audience"} onChange={handleRoleChange} />
              Audience
            </label>
          </div>
        )}
